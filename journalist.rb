...
@twitter = twitter
puts "Il y'a #{@twitter.size} journalistes."
def numberCount
  counter = 0
  @twitter.map { |x|
    if x.scan(/[0123456789]/).length > 0
     ...
     puts "Il y'a #{counter} handles contenant un numéro."
end
numberCount
...
def numberAude
  ...
  puts "Il y'a #{counter} handles contentant les 4 lettres du prénom 'Aude' "
end
numberAude
...
counter += 1
    ...
    puts "Il y'a #{counter} handles commençant par une majuscule."
  end
  beginupper
  ...
  def uppercount
    counter1 = 0
    @twitter.map { |i|
      if i.scan(/[ABCDEFGHIJKLMNOPQRSTUVWXYZ]/).length > 0
        counter1 += 1
    end
    }
    puts "Il y'a #{counter1} handles contenant une majuscule"
    end
    uppercount
